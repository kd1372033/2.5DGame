...
void TitleObj::Init()
{
	...
	m_enter.Load("Asset/Textures/Enter1.png");
	m_player.Load("Asset/Textures/Player.png");
	operate.Load("Asset/Textures/Operate1.png");
...
void TitleObj::DrawSprite()
{
	...
	if (m_opeCnt >= 0)
	{
		Math::Rectangle rc = { 561 * m_opeCnt, 0, 561, 720 };
		KdShaderManager::Instance().m_spriteShader.SetMatrix(operatemat);
		KdShaderManager::Instance().m_spriteShader.DrawTex(&operate, (int)m_operatepos.x, (int)m_operatepos.y, 561, 720, &rc);
	}
