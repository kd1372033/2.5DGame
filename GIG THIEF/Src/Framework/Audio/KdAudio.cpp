...
// BGMを最初から鳴らし直すための専用関数
void KdAudioManager::PlayBGM(std::string_view rName, float volume)
{
	// 1. すでにBGMが設定されていれば、一度確実にストップさせて破棄
	// これにより、2回目以降に戻ってきたときに必ず「最初から」再生されます
	if (m_currentBGM)
	{
		m_currentBGM->Stop();
	...
	// 2. 通常のPlay関数を使って新しく再生
	m_currentBGM = Play(rName, true); // ループ再生
